...
@while((have_rows('lay_sections', $option)))
  ...
  @if(get_sub_field('bool_banner', $option) && !get_sub_field('bool_banner_contain', $option))<div {{ Kernl\Layout::structure('banner', ['class' => '']) }} data-acf="bool_banner">@endif

    <section {{ Kernl\Layout::structure('section', ['class' => 'section']) }}
      id="section-{{ $i_section }}"
      data-section="{{ $i_section }}" data-acf="txt_class|etc">

      @if(get_sub_field('bool_banner', $option) && get_sub_field('bool_banner_contain', $option))<div {{ Kernl\Layout::structure('banner', ['class' => 'section']) }} data-acf="bool_banner">@endif
